...
class Worker:
    ...
    async def start(self) -> None:
        logger.info(f"Worker {self.worker_id} starting — exchange={self.exchange} pairs={'ALL' if self._pairs_mode == 'all' else self.pairs}")
    ...
    async def _resolve_all_pairs(self) -> List[str]:
        ...
        Quote filter (env PAIRS_ALL_QUOTE_FILTER) limits which quote currencies
        are included — defaults to USDT,USDC,BTC,ETH.
        """
        ...
        try:
            ...
            ex = getattr(ccxt_async, exchange_lower)({"enableRateLimit": True})
            ...
            if len(pairs) > self._MAX_AUTO_PAIRS:
                logger.warning(
                    f"[ALL] {self.exchange}: {len(pairs)} pairs found — "
                    f"capping at {self._MAX_AUTO_PAIRS}. "
                    f"Use PAIRS_ALL_QUOTE_FILTER or explicit PAIRS= to narrow."
                )
                pairs = pairs[:self._MAX_AUTO_PAIRS]
