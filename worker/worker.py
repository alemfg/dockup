...
class Worker:
    ...
    async def _collect_and_publish(self) -> None:
        """
        Fetch prices and publish to message bus.
        v6.7: Uses fetch_tickers() (one batch call) instead of fetch_ticker()
        per pair. Reduces exchange requests from N sequential HTTP calls to 1,
        eliminating the serial-loop stale problem and cutting CPU by ~99%.
        Falls back to per-pair if exchange does not support batch.
        """
        if self._paused:
            return
        if self._ccxt_client:
            # Check if batch was disabled for this exchange (runtime fallback)
            if getattr(self._ccxt_client, "_arbx_no_batch", False):
                await self._collect_per_pair()
            else:
    ...
    async def _collect_batch(self) -> None:
        """Live mode: one fetch_tickers() call for all tracked pairs."""
        t0 = time.time()
        try:
            await self._rate_guard.acquire()
            tickers = await self._ccxt_client.fetch_tickers(self.pairs)
            ...
            if any(x in exc_str for x in ("not supported", "notsupported", "badrequest", "invalid request")):
                logger.info(f"[{self.exchange}] fetch_tickers not supported, switching to per-pair")
                self._ccxt_client._arbx_no_batch = True
                await self._collect_per_pair()
                return
            self._handle_error(exc)
            return
        ...
        for pair in self.pairs:
            ...
            if not price:
                continue
            self._latency_samples.append(batch_latency / max(len(self.pairs), 1))
            ...
            published += 1
        if published == 0 and len(self.pairs) > 0:
            logger.warning(f"[{self.exchange}] fetch_tickers returned 0 pairs, switching to per-pair")
            self._ccxt_client._arbx_no_batch = True
            await self._collect_per_pair()

    async def _collect_per_pair(self) -> None:
        """Fallback: sequential fetch_ticker() for exchanges without batch support."""
        now_ts = datetime.now(timezone.utc).isoformat()
        for pair in self.pairs:
            t0 = time.time()
            try:
            ...
            if price:
                ...
                if len(self._latency_samples) > 60:
                    self._latency_samples.pop(0)
                await self._bus.publish_price_tick({
                    "worker_id":  self.worker_id, "exchange": self.exchange,
                    "pair": pair, "price": price,
                    "latency_ms": round(latency_ms, 2), "timestamp": now_ts,
                })
                self._ticks_this_minute += 1

    async def _collect_simulated(self) -> None:
        """Simulation mode — no exchange calls."""
        base_prices = {
        ...
        for pair in self.pairs:
            ...
            price = round(base * (1 + noise), 6)
            await self._bus.publish_price_tick({
                "worker_id": self.worker_id, "exchange": self.exchange,
                "pair": pair, "price": price, "latency_ms": 0.0, "timestamp": now_ts,
            })
            self._ticks_this_minute += 1

    async def _fetch_price(self, pair: str) -> Optional[float]:
        """Single-pair price fetch — used by per-pair fallback mode only."""
        if self._ccxt_client:
            await self._rate_guard.acquire()
            try:
                ticker = await self._ccxt_client.fetch_ticker(pair)
                ...
                if "429" in exc_str or "rate limit" in exc_str or "too many requests" in exc_str:
                    await self._rate_guard.on_rate_limited()
                raise exc
        base_prices = {"BTC/USDT": 65000.0, "ETH/USDT": 3200.0, "ADA/USDT": 0.58}
        base  = base_prices.get(pair, 100.0)
    ...
    async def _order_sync_loop_once(self) -> None:
        """Run one order sync cycle immediately (used by manual sync command)."""
        try:
                ...
                try:
                    ...
                    for o in open_orders:
                        await self._bus.publish_order_result({
                            ...
                            "exchange_order_id": o.get("id", ""), "status": o.get("status", "open"),
                            "filled_price": o.get("average") or o.get("price", 0.0),
                            "filled_volume": o.get("filled", 0.0), "side": o.get("side", ""),
                            "pair": o.get("symbol", ""), "source": "manual_sync",
                            "timestamp": datetime.now(timezone.utc).isoformat(),
                        })
                except Exception:
                    pass
            logger.info(f"[SYNC] Manual order sync complete for {self.exchange}")
        except Exception as exc:
            logger.warning(f"[SYNC] Manual sync error: {exc}")
    ...
    async def _command_listener(self) -> None:
        ...
        async def handler(stream: str, msg: dict) -> None:
            ...
            if cmd == "kill":
                ...
                # v6.5: Force immediate order sync cycle
                asyncio.create_task(self._order_sync_loop_once())
                logger.info(f"Manual order sync triggered on {self.worker_id}")

            elif cmd == "fetch_balance":
                # v6.5: Force immediate balance fetch
                asyncio.create_task(self._fetch_and_publish_balances())
                logger.info(f"Manual balance fetch triggered on {self.worker_id}")
