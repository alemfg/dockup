.PHONY: help install dev brain worker worker-file worker-pipe \
        manager frontend test test-cov test-quick test-full test-report \
...
test-quick:  ## Quick diagnostic (bash, no deps) — auto-detects port
	@bash scripts/test_quick.sh $(BRAIN_API_PORT)

test-full:   ## Full test suite (Python) — all 20 feature groups
	@python3 scripts/test_all.py --port $(BRAIN_API_PORT) --verbose

test-report: ## Full test suite with JSON report saved to test_report.json
	@python3 scripts/test_all.py --port $(BRAIN_API_PORT) --json test_report.json

data-dirs:
	@# Create host data directories for postgres and redis bind mounts.
	@# Uses ARBX_DATA_DIR from config/.env.local if set, otherwise ./data
	@DATA_DIR=$$(grep -s '^ARBX_DATA_DIR=' config/.env.local | cut -d= -f2- | tr -d '"' | tr -d "'"); \
	  DATA_DIR=$${DATA_DIR:-./data}; \
	  echo "  Data directory: $$DATA_DIR"; \
	  mkdir -p "$$DATA_DIR/postgres" "$$DATA_DIR/redis"; \
	  echo "  Created: $$DATA_DIR/postgres  $$DATA_DIR/redis"
...
docker-down:
	@echo "  [1/5] Stopping brain + worker_manager first (prevents AutoSpawner re-spawning)..."
	@docker stop arb-brain arb-worker-manager 2>/dev/null || true
	@echo "  [2/5] Stopping auto-spawned worker containers..."
	@docker ps -a --filter ancestor=arbx-worker:latest --format "{{.Names}}" \
	  | xargs -r docker stop 2>/dev/null || true
	@docker ps -a --filter ancestor=arbx-worker:latest --format "{{.Names}}" \
	  | xargs -r docker rm -f 2>/dev/null || true
	@echo "  [3/5] Flushing Redis streams..."
	...
	@echo "  [4/5] Stopping Compose services..."
	@docker compose -f docker/docker-compose.yml down 2>/dev/null || true
	@echo "  [5/5] Final cleanup — killing any remaining workers and removing network..."
	@REMAINING=$$(docker ps -aq --filter ancestor=arbx-worker:latest 2>/dev/null); \
	  if [ -n "$$REMAINING" ]; then \
	    echo "    Force-killing $$(echo $$REMAINING | wc -w) remaining worker(s)..."; \
	    echo $$REMAINING | xargs docker kill 2>/dev/null || true; \
	    echo $$REMAINING | xargs docker rm -f 2>/dev/null || true; \
	  fi
	@docker network rm arb-net 2>/dev/null && echo "    Network arb-net removed." || true
	@echo "  Done."
...
docker-ps:
	...
	@echo ""
	@echo "  Auto-spawned workers:"
	@docker ps --filter ancestor=arbx-worker:latest --format "  {{.Names}}\t{{.Status}}\t{{.RunningFor}}" 2>/dev/null || echo "  (none)"

diagnose:
	@echo ""
	@echo "  ══════════════════════════════════════════════════════════"
	@echo "  ARBX Diagnostics"
	@echo "  ══════════════════════════════════════════════════════════"
	@echo ""
	@echo "  ── Docker containers ────────────────────────────────────"
	@docker compose -f docker/docker-compose.yml ps --format "  {{.Name}}\t{{.Status}}" 2>/dev/null
	@echo ""
	@echo "  ── Worker containers ────────────────────────────────────"
	@docker ps --filter ancestor=arbx-worker:latest --format "  {{.Names}}\t{{.Status}}" 2>/dev/null || echo "  (none running)"
	@echo ""
	@echo "  ── Redis stream lengths ─────────────────────────────────"
	@docker exec arb-redis redis-cli XLEN stream:heartbeats 2>/dev/null | xargs -I{} echo "  stream:heartbeats   {}" || echo "  (redis unavailable)"
	@docker exec arb-redis redis-cli XLEN stream:prices 2>/dev/null | xargs -I{} echo "  stream:prices       {}" || true
	@docker exec arb-redis redis-cli XLEN stream:spawn_requests 2>/dev/null | xargs -I{} echo "  stream:spawn_requests {}" || true
	@echo ""
	@echo "  ── Consumer groups on heartbeats ────────────────────────"
	@docker exec arb-redis redis-cli XINFO GROUPS stream:heartbeats 2>/dev/null | grep -E "name|lag|pending" | sed 's/^/  /' || echo "  (stream empty)"
	@echo ""
	@echo "  ── Brain API health ─────────────────────────────────────"
	@curl -sf http://localhost:$(BRAIN_API_PORT)/api/health 2>/dev/null && echo " ✓ API responding" || echo "  ✗ API not responding on port $(BRAIN_API_PORT)"
	@echo ""
	@echo "  ── Brain API config ─────────────────────────────────────"
	@curl -sf http://localhost:$(BRAIN_API_PORT)/api/config/current 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'  version: {d.get(\"version\")}'); print(f'  exchanges: {d.get(\"workers\",{}).get(\"exchanges\",[])}'); print(f'  trading_mode: {d.get(\"trading_mode\")}')" 2>/dev/null || echo "  ✗ Could not fetch config"
	@echo ""
	@echo "  ── Fleet workers ────────────────────────────────────────"
	@curl -sf http://localhost:$(BRAIN_API_PORT)/api/fleet/workers 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); ws=d.get('workers',[]); print(f'  total: {len(ws)}'); [print(f'  {w[\"worker_id\"]}: {w[\"status\"]} ({w[\"exchange\"]})') for w in ws]" 2>/dev/null || echo "  ✗ Could not fetch fleet"
	@echo ""
