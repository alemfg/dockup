...
import { fetchSystemStatus, usePolling } from './utils/api'
...
function useVersion() {
  // Uses the same fetchSystemStatus as StatusBar — consistent and axios-based
  const { data } = usePolling(fetchSystemStatus, 30000)
  return data?.version ?? null
}

function VersionTag({ ver }) {
  if (!ver) return <span className="font-mono opacity-40 text-xs">...</span>
  return <span className="font-mono opacity-70">{ver}</span>
}

function VersionBadge({ ver }) {
  // Shows just the major.minor (e.g. "4.7") in the icon badge
  if (!ver) return <span>v4</span>
