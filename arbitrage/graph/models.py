...
class ArbitragePath:
    ...
    def gross_product(self) -> float:
        ...
        Sanity checks:
        - Any edge with effective_rate > 200 is almost certainly a unit
          mismatch (e.g. LBank/MEXC reporting KAVA/USDT ≈ 99.9 when real
          price is ~$0.6). Reject the whole path to stop phantom 50% cycles.
        - If -log_sum > 3 (≈ 20x gain) the composition is bogus. Reject.
        - Final result clamped to [0, 1.05]: no real CEX triangular arb
          exceeds ~5% gross; new-listing spikes are handled by listings module.
        """
        import math
        # Per-edge sanity: any implausible rate kills the path
        for e in self.edges:
            er = e.effective_rate
            if er <= 0 or er > 200:
                return 0.0
        log_sum = sum(e.log_weight for e in self.edges)  # sum of -log(er)
        # Guard: product = exp(-log_sum); reject if obviously bogus
        if -log_sum > 3:    # exp(3) ≈ 20x — not a real arb
            return 0.0
        try:
            product = math.exp(-log_sum)
        except OverflowError:
            return 0.0
        # Tight clamp: real CEX triangular arb rarely exceeds 2-3%
        return max(0.0, min(1.05, product))
