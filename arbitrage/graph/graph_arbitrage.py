...
class GraphArbitrage:
    ...
    async def scan(self) -> List[ArbitragePath]:
        ...
        stale_s = self._gcfg.stale_edge_s
        for path in valid_paths[:3]:
            # Compute max edge age for transparency in logs
            max_age_s = max(
                (time.time() - e.timestamp.timestamp()) for e in path.edges
            ) if path.edges else 0
            stale_warn = f" ⚠ STALE({max_age_s:.0f}s)" if max_age_s > stale_s * 0.8 else ""
            logger.info(
        ...
        # Emit financial events for new profitable cycles
        try:
            ...
            for path in valid_paths[:5]:
                ...
                lvl = Level.SUCCESS if net_pct >= 0.5 else Level.INFO
                asyncio.create_task(emit(
                    category = Category.ARBIT,
                    title    = f"Cycle detected: {path.path_string}",
                    detail   = (
                        f"{path.hops} hops · {', '.join(path.exchanges_used)} · "
                        f"+{net_pct:.3f}% net · ${net_usd:.2f}"
                    ),
                    level    = lvl,
                    data     = {
                        ...
                        "gross_pct": round(path.gross_profit_pct, 4),
            ...
            pass  # never let event publishing crash the scan
