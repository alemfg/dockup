...
class OrderLog:
    ...
    def record_close(self, order_id: str, close_reason: str, pnl_usd: float, close_price: float,
                     persistence=None) -> None:
        """Record that a position closed (SL/TP/manual). Optionally persists P&L to DB."""
        ...
        # Persist updated order and daily P&L to DB
        if persistence:
            persistence.persist_order(order)
            today = date.today().isoformat()
            closed = [o for o in self._orders.values() if o.get("status") == "closed"
                      and (o.get("closed_at") or "").startswith(today)]
            wins = sum(1 for o in closed if o.get("pnl_usd", 0) > 0)
            fees = sum(o.get("fees_paid", 0) for o in closed)
            realized = sum(o.get("pnl_usd", 0) for o in closed)
            persistence.persist_daily_pnl(today, realized, len(closed), wins, fees)
