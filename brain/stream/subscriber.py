...
class StreamSubscriber:
    ...
    def __init__(
        ...
        self._persistence     = None   # injected by brain after init
    ...
    async def run(self) -> None:
        await self._bus.subscribe(
            ...
            handler=self._route,
            batch_size=1,   # yield to event loop between every message
        )
    ...
    async def _on_order_result(self, msg: dict) -> None:
        ...
        if self._order_log:
            ...
            rec = self._order_log.get(order_id)
            if rec and self._persistence:
                self._persistence.persist_order(rec)
