...
class FleetMonitor:
    ...
    async def _consume_heartbeats(self) -> None:
        ...
        # Create consumer group reading from now ($) — stream is flushed on docker-down
        # so old messages won't pile up. Workers send heartbeats every 5s so we
        # get current state within seconds.
        try:
            await r.xgroup_create(STREAM_HEARTBEATS, group, id="$", mkstream=True)
        except Exception:
