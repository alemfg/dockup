...
class FleetMonitor:
    ...
    async def _consume_heartbeats(self) -> None:
        """
        Consume heartbeats using a dedicated consumer group ("fleet_monitor")
        so every heartbeat is guaranteed to reach this monitor regardless of
        what other consumers (brain_subscriber) are doing with the same stream.
        """
        group = "fleet_monitor"
        consumer_id = "fleet_monitor_1"

        # Wait for Redis connection to be ready (bus.connect() may not have run yet)
        for attempt in range(30):
            if self._bus._redis is not None:
                break
            await asyncio.sleep(0.5)
        else:
            logger.error("Fleet monitor: Redis connection never became available")
            return

        r = self._bus._redis

        # Create consumer group reading from recent past, not just new messages.
        # Workers that started BEFORE the brain will have published heartbeats
        # that we'd miss with id="$". We use id="0" but skip messages older
        # than 120s so we don't replay ancient history on restarts.
        try:
            await r.xgroup_create(STREAM_HEARTBEATS, group, id="0", mkstream=True)
        except Exception:
            pass  # group already exists — that's fine

        try:
            stream_len = await r.xlen(STREAM_HEARTBEATS)
            logger.info(f"Fleet monitor: consuming heartbeats as group '{group}' — stream has {stream_len} messages")
        except Exception:
            logger.info(f"Fleet monitor: consuming heartbeats as group '{group}'")
                ...
                for _stream, messages in results:
                    for msg_id, fields in messages:
                        try:
                            ...
                            logger.error(f"Heartbeat processing error: {exc}")
            except asyncio.CancelledError:
                break
            except Exception as exc:
                logger.error(f"Fleet monitor heartbeat loop error: {exc}")
