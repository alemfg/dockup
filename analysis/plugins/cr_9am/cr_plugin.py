...
class CR9AMPlugin(AnalysisPlugin):
    ...
    def __init__(self):
        ...
        self._signal_history: list = []   # persistent ring buffer of past signals
    ...
    def set_force_active(self, value: bool) -> None:
        ...
        try:
            ...
            emit_sync(
                category = Category.CR9AM,
                title    = "9AM analysis forced ON" if value else "9AM analysis restored to schedule",
                detail   = "Triggered manually via dashboard." if value else "Time-gate re-enabled.",
                level    = Level.WARNING if value else Level.INFO,
            )
        except Exception:
            pass
    ...
    async def run(self, exchange, pair, prices, candles=None, extra=None):
        ...
        # Emit financial event for this CR signal
        try:
            from events.event_bus import emit_sync, Category, Level
            emit_sync(
                category = Category.CR9AM,
                title    = f"CR Signal: {pair} {signal.direction.upper()}",
                detail   = (
                    f"Model={validated['model_type']} · Score={signal.setup_score:.2f} · "
                    f"Entry={signal.entry_price:.5f} · TP1={signal.tp1:.5f} · "
                    f"SL={signal.stop_loss:.5f} · {signal.confidence}"
                ),
                level    = Level.SUCCESS if signal.setup_score >= 0.7 else Level.INFO,
                data     = signal.to_dict(),
            )
        except Exception:
            pass
        ...
        self._record_to_history(sig_dict, action="signal_generated")
        return PluginResult(plugin=self.name, signal_score=signal.setup_score,
                            weight=self._weight, data=sig_dict, signal_type="cr_9am_setup")

    def get_active_signals(self):
        return [s["signal"].to_dict() for s in self._state.values() if s.get("signal")]

    def get_signal_history(self, limit: int = 100) -> list:
        """Return past CR signals newest-first, up to limit."""
        return list(reversed(self._signal_history))[:limit]

    def _record_to_history(self, signal_dict: dict, action: str = "signal_generated") -> None:
        """Append a signal event to the history ring buffer."""
        ...
        entry = {
            ...
            "score":     signal_dict.get("quality", {}).get("score", 0),
            "confidence":signal_dict.get("quality", {}).get("confidence", ""),
            "entry":     signal_dict.get("entry", {}).get("price", 0),
            "tp1":       signal_dict.get("targets", {}).get("tp1", 0),
            "tp2":       signal_dict.get("targets", {}).get("tp2", 0),
            "stop_loss": signal_dict.get("targets", {}).get("stop_loss", 0),
            "signal":    signal_dict,
        }
        self._signal_history.append(entry)
        if len(self._signal_history) > 500:
            self._signal_history = self._signal_history[-400:]
