...
class ContextEngine:
    ...
    def __init__(self, analysis_config: dict):
        ...
        # Analysis whitelist — only run plugins on these pairs.
        # Populated from ANALYSIS_PAIRS env var (comma-separated).
        # If empty, all pairs are analysed (only safe with small pair counts).
        _raw = os.getenv("ANALYSIS_PAIRS", "").strip()
        self._analysis_pairs: set = (
            {p.strip().upper() for p in _raw.split(",") if p.strip()}
            if _raw else set()
        )
        if self._analysis_pairs:
            logger.info(f"Analysis whitelist: {len(self._analysis_pairs)} pairs "
                        f"(set ANALYSIS_PAIRS=ALL to analyse everything)")
        else:
            logger.warning("ANALYSIS_PAIRS not set — plugins will run on ALL streamed "
                           "pairs. Set ANALYSIS_PAIRS=BTC/USDT,ETH/USDT,... to limit.")
    ...
    async def on_price_tick(
        ...
        Analysis plugins only run on pairs in ANALYSIS_PAIRS (default: top 20
        liquid pairs). All other pairs are tracked for graph arbitrage only —
        no plugin overhead. This prevents event loop saturation when workers
        stream hundreds of pairs.
        """
        ctx = self._get_or_create(exchange, pair)
        ctx.price_history.append(price)
        ctx.tick_count += 1

        # Skip plugin analysis for pairs not in the analysis whitelist
    ...
    def _should_analyse(self, pair: str) -> bool:
        """Return True if this pair should have plugins run on it."""
        if not self._analysis_pairs:
            return True  # no filter configured — analyse everything (old behaviour)
        return pair in self._analysis_pairs
