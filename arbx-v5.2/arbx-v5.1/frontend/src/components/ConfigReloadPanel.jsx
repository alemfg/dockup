...
function ExchangeCard({ exchange, onRemove, onEdit }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex flex-col gap-2">
      ...
      <button onClick={() => onEdit(exchange)} className="text-[10px] text-gray-600 hover:text-gray-400 text-left transition-colors">✏ Edit pairs</button>
    </div>
  )
}
    ...
    <div className="space-y-3">
      ...
      {section === 'risk' && cfg && (
        <Card title="Risk Manager" subtitle="Circuit breakers — hover a field and click Edit to change live">
          ...
          <EditableField label="Max open positions"  section="risk" fieldKey="max_open_positions"     value={cfg.risk?.max_open_positions}     type="int"               tooltip="Maximum simultaneous open positions across all pairs." />
        </Card>
      )}
