...
async def sync_orders(request: Request):
    """
    Force-sync open orders from all exchanges immediately.
    Broadcasts an order_sync command to all live workers.
    """
    brain   = request.app.state.brain
    ...
    return {
        "ok":      True,
        "message": f"Order sync command sent to {sent} worker(s). Data updates within ~5s.",
        "workers": sent,
    }
