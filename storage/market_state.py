...
class MarketState:
    ...
    def get_open_position_usd(self, exchange: str, pair: str) -> float:
        """
        Return the total USD value of open (unfilled) positions for a given
        exchange+pair combination.  Positions are recorded via record_order_result
        with status='open'.  Returns 0.0 if no open positions exist (safe default
        so risk checks don't crash when no position tracker is populated yet).
        """
        total = 0.0
        for result in self._order_results:
            if (
                result.get("exchange") == exchange
                and result.get("pair") == pair
                and result.get("status") in ("open", "pending", "partial")
            ):
                total += float(result.get("capital_usd", 0.0))
        return total
