...
import { usePolling, fetchVaultApiKeys, fetchVaultWallets, fetchVaultFees, fetchVaultWithdrawalFees } from '../utils/api'
...
function ApiKeysSection() {
  const { data, loading, error, refresh } = usePolling(fetchVaultApiKeys, 15000)
...
function WalletsSection() {
  const { data, loading, refresh } = usePolling(fetchVaultWallets, 15000)
...
function FeesSection() {
  const { data, loading, refresh } = usePolling(fetchVaultFees, 15000)
...
function WithdrawalFeesSection() {
  const { data, loading, refresh } = usePolling(fetchVaultWithdrawalFees, 15000)
  const fees = data?.fees ?? []
  const { sorted, col, dir, toggle } = useSortable(fees, 'exchange', 'asc')
