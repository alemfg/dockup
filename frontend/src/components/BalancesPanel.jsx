...
export default function BalancesPanel() {
  ...
  const { data: activeData } = usePolling(
    () => fetch('/api/config/exchanges/active').then(r => r.json()), 30000
  )
  ...
  const configuredExchanges = (activeData?.exchanges ?? []).map(e => e.exchange)
  const balanceExchanges    = Object.keys(byExchange)
  const allExchanges = [...new Set([...configuredExchanges, ...balanceExchanges])].sort()
  ...
  return (
    <div className="space-y-3">
      ...
      {section === 'overview' && (
        <Card title="Fund Balances" subtitle="All configured exchanges — missing balance means no API key or worker not running">
          {allExchanges.length === 0 ? (
            <EmptyState message="No exchanges configured — add in Config → Exchanges" icon="💰" />
          ) : (
            <div className="space-y-1">
              {allExchanges.map(ex => {
                const bals   = byExchange[ex]
                const exInfo = (activeData?.exchanges ?? []).find(e => e.exchange === ex)
                const hasApiKey = exInfo?.authenticated ?? false
                const hasWorker = (exInfo?.worker_count ?? 0) > 0
                const isLive    = (exInfo?.healthy ?? 0) > 0

                if (!bals || bals.length === 0) {
                  return (
                    <div key={ex} className="flex items-center gap-3 px-3 py-2.5 border border-gray-800 rounded-lg bg-gray-900/30">
                      <span className={`w-2 h-2 rounded-full shrink-0 ${isLive ? 'bg-green-500' : hasWorker ? 'bg-yellow-500' : 'bg-gray-600'}`} />
                      <span className="text-sm font-medium text-gray-300 capitalize w-28 shrink-0">{ex}</span>
                      <span className="flex-1 text-xs text-gray-600">
                        {!hasApiKey
                          ? '🔑 No API key — add credentials in Vault to fetch balances'
                          : !hasWorker
                          ? '⚠ No worker running'
                          : isLive
                          ? '⟳ Worker active — click Sync Balances to fetch'
                          : '⏳ Worker starting…'}
                      </span>
                      <span className="text-xs text-gray-700 font-mono shrink-0">$—</span>
                    </div>
                  )
                }
                return (
                  <ExchangeBar key={ex} exchange={ex} balances={bals}
                    totalUsd={totalUsd} targetPct={targetMap[ex] ?? null} />
                )
              })}
            </div>
          )}
      ...
      {section === 'targets' && (
        <Card title="Target Allocation" subtitle="Saved to DB — used by Rebalance analyser">
          {allExchanges.length === 0 ? (
            <EmptyState message="No exchanges configured yet" icon="🎯" />
          ) : (
            <TargetEditor exchanges={allExchanges} />
          )}
        </Card>
      )}
      ...
      {section === 'rebalance' && (
        <Card title="Rebalance" subtitle="Requires API keys and wallet addresses in Vault">
          <RebalancePanel totalUsd={totalUsd} exchanges={allExchanges} />
        </Card>
      )}
    </div>
  )
