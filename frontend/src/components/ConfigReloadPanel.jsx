...
function ExchangeManager() {
  ...
  // Add computed status_text for sorting
  const activeWithStatus = active.map(ex => ({
    ...ex,
    status_text: ex.healthy > 0 ? 'live' : ex.worker_count > 0 ? 'starting' : 'offline',
  }))
  const { sorted: sortedActive, col: exCol, dir: exDir, toggle: exToggle } = useSortable(activeWithStatus, 'exchange', 'asc')
          ...
          <table className="w-full text-xs">
            <thead className="bg-gray-900 border-b border-gray-800">
              <tr>
                ...
                <SortTh col="exchange"     sortCol={exCol} sortDir={exDir} onSort={exToggle}>Exchange</SortTh>
                <th className="text-left py-2 px-2 text-gray-500 uppercase font-medium hidden sm:table-cell">Pairs</th>
                <SortTh col="worker_count" sortCol={exCol} sortDir={exDir} onSort={exToggle}>Workers</SortTh>
                <SortTh col="status_text"  sortCol={exCol} sortDir={exDir} onSort={exToggle}>Status</SortTh>
...
const SECTIONS = [
  ...
  { id: 'alert_cats', label: '📢 Alert Rules' },
  { id: 'cr9am',      label: '📐 9AM CR' },
  { id: 'brain',      label: '🧠 Brain' },
]

// ── Alert Categories Panel ─────────────────────────────────────────────────────

const ALL_CHANNELS = ['ntfy', 'slack', 'discord', 'telegram', 'email', 'webhook']
const PRIORITIES   = ['critical', 'high', 'default', 'low', 'min']

function AlertCategoriesPanel() {
  const { data, loading, error, refresh } = usePolling(
    () => fetch('/api/alerts/categories').then(r => r.json()), 30000
  )
  ...
  const patch = async (catId, changes) => {
    setSaving(s => ({ ...s, [catId]: true }))
    try {
      const r = await fetch(`/api/alerts/categories/${catId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(changes),
      }).then(x => x.json())
      if (r.ok) { setStatus({ ok: true, msg: `Saved ${catId}` }); refresh() }
      else { setStatus({ ok: false, msg: r.detail ?? 'Save failed' }) }
    } catch(e) { setStatus({ ok: false, msg: e.message }) }
    finally { setSaving(s => ({ ...s, [catId]: false })) }
    setTimeout(() => setStatus(null), 3000)
  }

  const reset = async () => {
    if (!confirm('Reset all alert rules to defaults?')) return
    await fetch('/api/alerts/categories/reset', { method: 'POST' })
    refresh()
  }

  if (loading) return <div className="text-xs text-gray-500 py-4 text-center">Loading alert rules…</div>
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-500">
          Control which events fire alerts and which channels receive them.
          Channels must be configured in the <strong className="text-gray-400">🔔 Alerts</strong> tab first.
        </p>
        <button onClick={reset}
          className="text-xs px-3 py-1 bg-gray-800 hover:bg-gray-700 text-gray-400 rounded border border-gray-700">
          Reset defaults
        </button>
      </div>
      ...
      {status && (
        <div className={`text-xs px-3 py-2 rounded border ${
          status.ok ? 'bg-green-900/20 border-green-800 text-green-300'
      ...
      <div className="space-y-2">
        {Object.entries(cats).map(([id, cat]) => (
          <div key={id} className={`border rounded-xl overflow-hidden transition-colors ${
            cat.enabled ? 'border-gray-700' : 'border-gray-800 opacity-60'}`}>
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-gray-900/50">
              ...
              {/* Enable toggle */}
              <button
                onClick={() => patch(id, { enabled: !cat.enabled })}
                disabled={saving[id]}
                className={`text-xs px-3 py-1.5 rounded border font-medium transition-colors ${
                  cat.enabled
                    ? 'border-green-700 bg-green-900/30 text-green-400 hover:bg-green-900/50'
                    : 'border-gray-700 bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                {saving[id] ? '⟳' : cat.enabled ? '● On' : '○ Off'}
              </button>
            </div>

            {/* Channel + priority config */}
            {cat.enabled && (
              <div className="px-4 py-3 border-t border-gray-800 bg-gray-950/40 space-y-2">
                {/* Channels */}
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] text-gray-500 w-16 shrink-0">Channels</span>
                  {ALL_CHANNELS.map(ch => {
                    const active = (cat.channels || []).includes(ch)
                    return (
                      <button key={ch}
                        onClick={() => {
                          const current = cat.channels || []
                          const next = active
      ...
      <p className="text-[10px] text-gray-600 pt-1">
        Note: These rules control when the brain fires alerts. Channel credentials and enabled state
        are set in <strong>🔔 Alerts</strong>. A category must be enabled AND its channel configured
...
export default function ConfigReloadPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      {section === 'alert_cats' && (
        <Card title="Alert Rules" subtitle="Configure which events trigger notifications and on which channels">
          <AlertCategoriesPanel />
        </Card>
      )}
