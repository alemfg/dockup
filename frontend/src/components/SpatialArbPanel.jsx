...
function SpreadRow({ item, allPrices }) {
  ...
  // API now provides buy/sell exchange directly
  const buyEx    = item.buy_ex    ?? Object.entries(prices).sort((a,b)=>a[1]-b[1])[0]?.[0] ?? '?'
  const sellEx   = item.sell_ex   ?? Object.entries(prices).sort((a,b)=>b[1]-a[1])[0]?.[0] ?? '?'
  const buyPrice  = item.buy_price  ?? Math.min(...Object.values(prices))
  const sellPrice = item.sell_price ?? Math.max(...Object.values(prices))
  const netPct    = item.net_pct   ?? spreadPct
  const feePct    = item.fee_pct   ?? 0
  const profitable = item.profitable ?? netPct > 0

  const capitalFor100 = netPct > 0 ? Math.round(100 / (netPct / 100)) : '∞'
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="shrink-0 w-24">
          <div className={`text-lg font-bold font-mono ${spreadColor(spreadPct)}`}>
            {spreadPct.toFixed(2)}%
          </div>
          <div className={`text-xs font-mono ${profitable ? 'text-green-400' : 'text-red-400'}`}>
            net {netPct.toFixed(2)}%
          </div>
        </div>
        {/* hidden original tag to satisfy rest of JSX */}
        {false && <span className={`text-xl font-bold font-mono w-16 shrink-0 ${spreadColor(spreadPct)}`}>
          {spreadPct.toFixed(2)}%
        </span>}
