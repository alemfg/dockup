...
const fetchAllSpreads = () => fetch('/api/market/all-pairs').then(r => r.json())
...
export default function SpatialArbPanel() {
  ...
  // Single poll — fetcher switches based on fullRange toggle
  const activeFetcher = fullRange ? fetchAllSpreads : fetchSpreads
  const { data: spreadData, loading } = usePolling(activeFetcher, fullRange ? 8000 : 5000)
  const { data: rlData }              = usePolling(fetchRateLimits, 10000)

  if (loading && !spreadData) return <Card title="Spatial Arbitrage"><Spinner /></Card>

  const rawSpreads = spreadData?.spreads ?? []
  const diag       = spreadData?.diag    ?? null

  // Full-range metadata
  const totalInMemory  = spreadData?.total_pairs_in_memory ?? null
  const pairsWithSpread = spreadData?.pairs_with_spread ?? null
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase">Pairs w/ spread</p>
          <p className="text-2xl font-bold text-white mt-1">{rawSpreads.length}</p>
          {fullRange && totalInMemory && (
            <p className="text-[10px] text-gray-600 mt-0.5">{totalInMemory} in memory</p>
          )}
        </div>
      ...
      {/* Rate limit monitor */}
      {(showRL || has429s) && rlData && <RateLimitPanel rlData={rlData} />}
      ...
      <Card>
        <div className="flex gap-2 flex-wrap items-center">
          ...
          {/* Full-range toggle */}
          <button
            onClick={() => setFullRange(r => !r)}
            title={fullRange
              ? 'Showing ALL pairs in memory — click to switch back to configured pairs only'
              : 'Showing configured pairs only (PAIRS_ALL_QUOTE_FILTER) — click to analyse ALL pairs in memory'}
            className={`text-xs px-2.5 py-1 rounded transition-colors flex items-center gap-1.5 border
              ${fullRange
                ? 'bg-purple-900/40 border-purple-700/60 text-purple-300 hover:bg-purple-900/60'
                : 'bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700'}`}
          >
            <span>{fullRange ? '🔮' : '⚙'}</span>
            <span>{fullRange ? 'Full range' : 'Configured pairs'}</span>
          </button>
        ...
        {/* Full-range info bar */}
        {fullRange && (
          <div className="mt-2 pt-2 border-t border-gray-800 flex items-center gap-2 text-[10px] text-gray-500">
            <span className="text-purple-400 font-semibold">Full range mode</span>
            <span>—</span>
            <span>Analysing all {totalInMemory ?? '…'} pairs in memory ({pairsWithSpread ?? '…'} have cross-exchange spreads). No quote filter applied.</span>
            <span className="ml-auto text-gray-600">Configured mode filters by PAIRS_ALL_QUOTE_FILTER</span>
          </div>
        )}
      </Card>
      ...
      {/* Spread list */}
      <Card
        title={fullRange ? 'Cross-Exchange Spreads — Full Range' : 'Cross-Exchange Spreads'}
