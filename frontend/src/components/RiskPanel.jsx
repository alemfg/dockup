...
function EditableField({ label, value, onSave, type = 'number', hint }) {
    ...
    <div className="space-y-4">
      ...
      <Card title="Circuit Breaker Status">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          {[
            { label: 'Daily Loss',  val: `$${status?.daily_loss_usd ?? 0}`, limit: `/ $${status?.max_daily_loss_usd ?? 0}`, pct: status?.daily_loss_pct ?? 0, col: 'bg-red-500' },
            { label: 'Drawdown',    val: `${status?.drawdown_pct ?? 0}%`,   limit: `/ ${status?.max_drawdown_pct ?? 0}%`,  pct: ((status?.drawdown_pct ?? 0) / Math.max(status?.max_drawdown_pct ?? 10, 0.1)) * 100, col: 'bg-orange-500' },
            { label: 'Consec. Losses', val: status?.consecutive_losses ?? 0, limit: `/ ${status?.max_consecutive ?? 5}`, pct: ((status?.consecutive_losses ?? 0) / Math.max(status?.max_consecutive ?? 5, 1)) * 100, col: 'bg-yellow-500' },
          ].map(g => (
            <div key={g.label}>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-gray-400">{g.label}</span>
                <span className="text-gray-300">{g.val} <span className="text-gray-600">{g.limit}</span></span>
              </div>
              <div className="w-full bg-gray-800 rounded-full h-2">
                <div className={`h-2 rounded-full ${g.col} transition-all`} style={{ width: `${Math.min(g.pct, 100)}%` }} />
              </div>
            </div>
          ))}
        </div>
        <p className="text-xs text-gray-600">Peak balance: ${status?.peak_balance_usd ?? 0}</p>
      </Card>
      ...
      <Card title="Risk Limits" subtitle="Changes apply immediately without restart">
        {rCfg && (
          <div>
            <EditableField label="Max Daily Loss (USD)"    value={rCfg.max_daily_loss_usd}
              onSave={v => updateRiskConfig({ max_daily_loss_usd: v })}
              hint="System halts when daily losses hit this" />
            <EditableField label="Max Drawdown (%)"        value={rCfg.max_drawdown_pct}
              onSave={v => updateRiskConfig({ max_drawdown_pct: v })}
              hint="% drop from peak balance" />
            <EditableField label="Max Consecutive Losses"  value={rCfg.max_consecutive_losses}
              onSave={v => updateRiskConfig({ max_consecutive_losses: v })} />
            <EditableField label="Max Pair Exposure (USD)" value={rCfg.max_pair_exposure_usd}
      ...
      {/* Decision engine rules — editable */}
      <Card title="Decision Engine Rules" subtitle="Controls when signals become orders">
        {dCfg && (
          <div>
            <EditableField label="CR Min Score"        value={dCfg.cr_min_score}
              onSave={v => updateDecisionConfig({ cr_min_score: v })}
              hint="0.0 – 1.0. Signal score must exceed this" />
            <EditableField label="CR Min Risk/Reward"  value={dCfg.cr_min_rr}
              onSave={v => updateDecisionConfig({ cr_min_rr: v })}
              hint="e.g. 1.5 = TP must be 1.5× the SL distance" />
            <EditableField label="Max Capital / Trade (USD)" value={dCfg.max_capital_per_trade}
              onSave={v => updateDecisionConfig({ max_capital_per_trade: v })} />
            <EditableField label="Max Concurrent Orders"    value={dCfg.max_concurrent_orders}
              onSave={v => updateDecisionConfig({ max_concurrent_orders: v })} type="number" />
            <EditableField label="Cooldown (seconds)"       value={dCfg.cooldown_seconds}
              onSave={v => updateDecisionConfig({ cooldown_seconds: v })}
              hint="Min time between orders on same pair" />
            <EditableField label="Graph Min Profit (%)"     value={dCfg.graph_min_profit_pct}
