...
function WorkerActionBtn({ label, title, onClick, variant = 'default', disabled }) {
  ...
  return (
    <button title={title} onClick={handle} disabled={disabled || loading}
...
function ApiModeBadge({ worker }) {
  const rl = worker.rate_limit ?? {}
  const cond = worker.condition ?? ''

  if (cond === 'ip_block' || worker.status === 'blocked') {
    return <span className="text-[9px] px-1.5 py-0.5 rounded border border-red-800 bg-red-900/20 text-red-400">⛔ blocked</span>
  }
  if (cond === 'rate_limit' || (rl.consecutive_429s ?? 0) > 0) {
    return <span className="text-[9px] px-1.5 py-0.5 rounded border border-yellow-800 bg-yellow-900/20 text-yellow-400">⚠ rate ltd</span>
  }
  if (cond === 'auth_error') {
    return <span className="text-[9px] px-1.5 py-0.5 rounded border border-red-800 bg-red-900/10 text-red-500">🔑 auth err</span>
  }
  if (worker.error_count > 5) {
    return <span className="text-[9px] px-1.5 py-0.5 rounded border border-orange-800 bg-orange-900/10 text-orange-400">⚠ {worker.error_count} err</span>
  }
  if (!worker.last_error) {
    return <span className="text-[9px] px-1.5 py-0.5 rounded border border-green-800 bg-green-900/10 text-green-500">✓ ok</span>
  }
  return <span className="text-[9px] px-1.5 py-0.5 rounded border border-gray-700 text-gray-500">—</span>
}
    ...
    <tr className={`border-b border-gray-800/40 hover:bg-gray-800/30 transition-colors ${isDead ? 'opacity-50' : ''}`}>
      <td className="py-2.5 pr-3">
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={() => onSelect(worker.worker_id)}
            className="text-sm text-blue-400 hover:text-blue-300 font-mono text-left">
            {worker.worker_id}
          </button>
          {worker.is_replacement && (
            <span className="text-xs bg-purple-900/40 text-purple-400 border border-purple-800 px-1 rounded">replacement</span>
          )}
          {isPaused && (
            <span className="text-xs bg-yellow-900/40 text-yellow-400 border border-yellow-800 px-1 rounded">⏸ paused</span>
          )}
        </div>
      </td>
      <td className="py-2.5 pr-3 text-gray-300 text-sm capitalize">{worker.exchange}</td>
      <td className="py-2.5 pr-3 text-gray-500 text-xs font-mono">{worker.pair_count ?? (worker.pairs || []).length}</td>
      ...
      <td className="py-2.5 pr-3"><ApiModeBadge worker={worker} /></td>
      <td className="py-2.5">
...
export default function FleetHealth({ onSelectWorker }) {
  ...
  // Flatten workers for sortable hook — status_order is a virtual sort key
  const rawWorkers = (data?.workers ?? []).map(w => ({
    ...w,
    status_order: STATUS_ORDER[w.status] ?? 9,
  }))

  const { sorted: workers, col, dir, toggle } = useSortable(rawWorkers, 'status_order', 'asc')
  const events = data?.events ?? []

  const healthy  = rawWorkers.filter(w => w.status === 'healthy').length
  const degraded = rawWorkers.filter(w => ['degraded', 'overloaded'].includes(w.status)).length
  const paused   = rawWorkers.filter(w => w.status === 'paused').length
  const blocked  = rawWorkers.filter(w => w.status === 'blocked').length
  const dead     = rawWorkers.filter(w => w.status === 'dead').length

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        <Card><Stat label="Total"    value={rawWorkers.length} color="text-white" /></Card>
      ...
      <Card
        title="Worker Fleet"
        subtitle={`${rawWorkers.length} registered workers`}
        action={
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  <SortTh col="worker_id"    sortCol={col} sortDir={dir} onSort={toggle}>Worker ID</SortTh>
                  <SortTh col="exchange"     sortCol={col} sortDir={dir} onSort={toggle}>Exchange</SortTh>
                  <SortTh col="pair_count"   sortCol={col} sortDir={dir} onSort={toggle}>Pairs</SortTh>
                  <SortTh col="status_order" sortCol={col} sortDir={dir} onSort={toggle}>Status</SortTh>
                  <SortTh col="latency_ms"   sortCol={col} sortDir={dir} onSort={toggle}>Latency</SortTh>
                  <th className="text-left text-xs text-gray-500 uppercase pb-2 pr-3 font-medium">API Health</th>
                  <th className="text-left text-xs text-gray-500 uppercase pb-2 font-medium">Actions</th>
                </tr>
              </thead>
              <tbody>
                {workers.map(w => (
