...
import { usePolling, fetchCRSignals, fetchCRDailyState, fetchCRHistory, setCRForceActive } from '../utils/api'
  ...
  return (
    <div className="mt-3 mb-1">
      ...
      <div className="relative h-8 bg-gray-800 rounded overflow-hidden">
        ...
        <div className="absolute h-full bg-blue-900/40 border-x border-blue-700"
          style={{ left: `${pct(ltf.low)}%`, width: `${pct(ltf.high) - pct(ltf.low)}%` }} />
        {entry && (
          <div className={`absolute h-full ${dc.bg} border-x ${dc.border}`}
            style={{ left: `${pct(entry.zone_bottom)}%`, width: `${pct(entry.zone_top) - pct(entry.zone_bottom)}%` }} />
        )}
        <div className="absolute inset-0 flex items-center justify-between px-1 text-xs">
          ...
          <span className="text-blue-400 font-medium">LTF {ltf.low?.toFixed(0)}–{ltf.high?.toFixed(0)}</span>
...
function SignalCard({ signal }) {
  ...
  return (
    <div className={`border ${dc.border} ${dc.bg} rounded-lg p-4 mb-3`}>
      ...
      <RangeVisual htf={signal.htf_range} ltf={signal.ltf_range} entry={signal.entry} direction={signal.direction} />
...
function HistoryRow({ entry }) {
  ...
  const dc = DIRECTION_COLORS[entry.direction] || DIRECTION_COLORS.long
  const ts = new Date(entry.ts)

  return (
    <div className={`border border-gray-800 rounded-lg mb-2 overflow-hidden`}>
      <div
        className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-gray-800/30"
        onClick={() => setOpen(o => !o)}
      >
        {/* Direction */}
        <span className={`font-bold text-sm w-4 ${dc.text}`}>{dc.icon}</span>

        {/* Pair + exchange */}
        <div className="min-w-[110px]">
          <span className="text-gray-200 text-xs font-semibold">{entry.pair}</span>
          <span className="text-gray-600 text-[10px] ml-1">{entry.exchange}</span>
        </div>

        {/* Score */}
        <span className="text-[10px] text-gray-500 font-mono w-14">
        ...
        {/* Entry / TP1 / SL */}
        <div className="hidden sm:flex gap-3 text-[10px] font-mono flex-1">
          <span className="text-gray-400">E: <span className="text-gray-200">{entry.entry?.toFixed(4)}</span></span>
          <span className="text-yellow-500">TP1: <span className="text-yellow-300">{entry.tp1?.toFixed(4)}</span></span>
          <span className="text-red-500">SL: <span className="text-red-300">{entry.stop_loss?.toFixed(4)}</span></span>
        </div>

        {/* Timestamp */}
        <div className="ml-auto text-[10px] text-gray-600 font-mono text-right shrink-0">
          <div>{ts.toLocaleTimeString('en-US', { hour12: false })}</div>
...
function HistorySection() {
  const { data, loading } = usePolling(fetchCRHistory, 30000)
  const history = data?.history ?? []

  if (loading) return <Spinner />

  return (
    <Card title="CR Signal History" subtitle={`${history.length} past signals recorded this session`}>
      {history.length === 0 ? (
        <EmptyState icon="🕘"
          message="No signal history yet. Signals are recorded when the CR 9AM plugin generates a setup. History resets on brain restart." />
      ) : (
      ...
      <p className="text-[10px] text-gray-700 mt-3">
        ⚠ History is in-memory and resets on brain restart. Persistent DB storage is on the roadmap.
      </p>
    </Card>
  )
}
          ...
          <div key={i} className="border border-gray-800 rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="bg-gray-800/50 rounded p-2">
                ...
                {s.range_1am ? (
                  <p className="font-mono text-gray-300">H: {s.range_1am.high?.toFixed(5)}<br/>L: {s.range_1am.low?.toFixed(5)}</p>
                ) : <p className="text-gray-600">Not built yet</p>}
              </div>
              <div className="bg-gray-800/50 rounded p-2">
                <p className="text-gray-500 mb-1">5AM Range</p>
                {s.range_5am ? (
                  <p className="font-mono text-gray-300">H: {s.range_5am.high?.toFixed(5)}<br/>L: {s.range_5am.low?.toFixed(5)}</p>
                ) : <p className="text-gray-600">Not built yet</p>}
              </div>
            </div>
            {s.sweep && (
...
export default function CRPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className={`text-lg font-bold ${isActive ? 'text-green-400' : isPreWindow ? 'text-yellow-400' : 'text-gray-500'}`}>
                {forceActive ? '⚡ FORCED ACTIVE' : isActive ? '● ACTIVE' : isPreWindow ? '○ PRE-WINDOW (8AM building)' : '○ INACTIVE'}
              </p>
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={toggleForce} disabled={forceLoading}
        ...
        <div className="text-right text-xs text-gray-500 mt-2">
          <p>8AM: Build HTF 1H range · 9:15: Build LTF 15M range · 9:00–10:00: Monitor sweeps</p>
        </div>
      </Card>
        ...
        {TABS.map(({ id, label }) => (
          <button key={id} onClick={() => setTab(id)}
            className={`text-xs px-3 py-1.5 rounded transition-colors ${
              tab === id ? 'bg-indigo-700 text-white font-medium' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      {/* Live signals */}
      {tab === 'live' && (
        <Card title="9AM CR Setups" subtitle={`${signals.length} active setup${signals.length !== 1 ? 's' : ''} today`}>
          {loading ? <Spinner /> : signals.length === 0 ? (
            <EmptyState
