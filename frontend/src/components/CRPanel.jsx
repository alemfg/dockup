...
import { usePolling, fetchCRSignals } from '../utils/api'
import { Card, ConfidenceBadge, EmptyState, Spinner, Stat } from './ui'
...
export default function CRPanel() {
  ...
  const isActive = nyHour >= 9 && nyHour < 10
  const isPreWindow = nyHour === 8
          ...
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className={`text-lg font-bold ${isActive ? 'text-green-400' : isPreWindow ? 'text-yellow-400' : 'text-gray-500'}`}>
                {isActive ? '● ACTIVE' : isPreWindow ? '○ PRE-WINDOW (8AM building)' : '○ INACTIVE'}
              </p>
            </div>
            <div>
