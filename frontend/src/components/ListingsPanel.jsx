...
function WatchlistSection() {
  const { data, loading, refresh } = usePolling(() => api('/listings/watchlist'), 10000)
...
function CGListingsSection() {
  const { data, loading, refresh } = usePolling(() => api('/listings/new?limit=30'), 30000)
...
export default function ListingsPanel() {
  ...
  const { data: oppData, loading: oppLoading } = usePolling(
    () => api('/listings/opportunities?limit=100'), 10000
  )
  const { data: statusData } = usePolling(() => api('/listings/status'), 30000)
