    ...
    <div className="border border-gray-800 rounded-xl overflow-hidden">
      <table className="w-full text-xs">
        <thead className="bg-gray-900 border-b border-gray-800">
          <tr>
            {['Pair','Exchange','Side','Type','Volume','Price','Mode','Status','Actions'].map(h => (
        ...
        <tbody>
          {orders.map(o => (
            <tr key={o.id} className="border-b border-gray-800/40 hover:bg-gray-800/20">
              ...
              <td className={`py-2 px-2 font-semibold ${o.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>
                {o.side?.toUpperCase()}
              </td>
              ...
              <td className="py-2 px-2">
                <span className={`text-[9px] px-1.5 py-0.5 rounded border ${
                  o.trading_mode === 'live' ? 'text-red-400 border-red-800 bg-red-900/20' :
      ...
      {orders.length === 0 ? (
        ...
        <div className="border border-gray-800 rounded-xl overflow-hidden">
          <table className="w-full text-xs">
            <thead className="bg-gray-900 border-b border-gray-800">
              <tr>
                {['Pair','Exchange','Side','Source','Status','P&L','Mode','Notes'].map(h => (
                  <th key={h} className="text-left py-2 px-2 text-gray-500 uppercase text-[10px]">{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {orders.map(o => {
                ...
                return (
                  <tr key={o.id} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                    ...
                    <td className={`py-1.5 px-2 font-semibold ${o.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>
                      {o.side?.toUpperCase()}
                    </td>
