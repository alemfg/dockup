  ...
  return (
    <tr className="border-b border-gray-800/40 hover:bg-gray-800/20 text-xs">
      ...
      <td className="py-2 pr-3 font-mono text-gray-300">
        {isOpen ? <span className="text-yellow-400">{trade.current_price?.toFixed(4)}</span>
                : trade.close_reason}
      </td>
      <td className="py-2 pr-3">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label="Total P&L"     value={`$${(ord.total_pnl_usd ?? 0).toFixed(2)}`}
          color={(ord.total_pnl_usd ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'} />
        <StatCard label="Win Rate"      value={`${ord.win_rate ?? 0}%`}
          sub={`${ord.wins ?? 0}W / ${ord.losses ?? 0}L`} color="text-blue-400" />
        <StatCard label="Total Trades"  value={ord.closed ?? 0}
          sub={`${ord.simulated ?? 0} simulated · ${ord.live ?? 0} live`} />
        <StatCard label="Daily Loss"    value={`$${risk.daily_loss_usd ?? 0}`}
      ...
      <Card title="Risk Status">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: 'Daily Loss', pct: risk.daily_loss_pct ?? 0, color: 'bg-red-500' },
            { label: 'Drawdown',   pct: ((risk.drawdown_pct ?? 0) / (risk.max_drawdown_pct || 10)) * 100, color: 'bg-orange-500' },
            { label: 'Consec. Losses', pct: ((risk.consecutive_losses ?? 0) / (risk.max_consecutive || 5)) * 100, color: 'bg-yellow-500' },
          ].map(g => (
            <div key={g.label}>
              <div className="flex justify-between text-xs text-gray-400 mb-1">
                <span>{g.label}</span><span>{Math.round(g.pct)}%</span>
              </div>
              <div className="w-full bg-gray-800 rounded-full h-2">
                <div className={`h-2 rounded-full ${g.color} transition-all`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Card title="Trade Stats">
            <div className="space-y-2 text-sm">
              {[
                ['Best Trade',    `$${ord.best_trade ?? 0}`],
                ['Worst Trade',   `$${ord.worst_trade ?? 0}`],
                ['Avg P&L',       `$${ord.avg_pnl_usd ?? 0}`],
                ['Total Fees',    `$${ord.total_fees_usd ?? 0}`],
              ].map(([l, v]) => (
          ...
          <Card title="Open Positions">
            {(summary?.positions?.total_trades === 0) ? (
              ...
              <div className="space-y-2 text-xs">
                {trades.filter(t => t.status === 'open' || t.status === 'tp1_hit').map((t, i) => (
                  <div key={i} className="flex justify-between border-b border-gray-800 pb-1">
                    <span className="text-gray-300">{t.pair} <span className="text-gray-600">{t.exchange}</span></span>
