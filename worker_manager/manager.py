...
async def run() -> None:
    ...
    # Use a dedicated consumer group so SPAWN messages are never stolen by
    # the brain's "brain" consumer group. Read from id="0" so messages
    # published before this process started are not missed.
    ...
    try:
        await r.xgroup_create(STREAM_SPAWN, group, id="0", mkstream=True)
        logger.info(f"Consumer group '{group}' created on {STREAM_SPAWN}")
    except Exception:
        logger.info(f"Consumer group '{group}' already exists on {STREAM_SPAWN}")

    consumer_id = f"wm_{MY_IP}"
    logger.info(f"Listening for SPAWN requests as {consumer_id} in group '{group}'")
    while True:
        try:
            ...
            for _stream, messages in results:
                for msg_id, fields in messages:
                    try:
                        ...
                        await handle_spawn_request(STREAM_SPAWN, data)
                        await r.xack(STREAM_SPAWN, group, msg_id)
                    except Exception as exc:
                        logger.error(f"SPAWN handler error: {exc}")
        except asyncio.CancelledError:
            break
        except Exception as exc:
            logger.error(f"WorkerManager loop error: {exc}")
