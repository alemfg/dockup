...
async def handle_spawn(msg: dict) -> None:
    ...
    exchange_upper = msg.get("exchange", "").upper()
    env_vars = {
        "WORKER_ID":       worker_id,
    ...
    # Inject Vault credentials if provided in spawn message (override env file)
    if msg.get("api_key"):
        env_vars[f"{exchange_upper}_API_KEY"]    = msg["api_key"]
    if msg.get("api_secret"):
        env_vars[f"{exchange_upper}_API_SECRET"] = msg["api_secret"]
    if msg.get("passphrase"):
        env_vars[f"{exchange_upper}_PASSPHRASE"] = msg["passphrase"]
