...
class AlertSender:
    ...
    def _get_session(self) -> aiohttp.ClientSession:
        """Return the shared session, creating a temporary one if not started yet.
        The shared session is preferred (connection-pooled). When called directly
        from the test route (bypassing the queue) the session may not exist yet —
        in that case we create a one-shot session for this call.
        Note: callers that create a temp session must close it themselves.
        """
        if self._session and not self._session.closed:
            return self._session
        # Fallback — create a temporary session (test path)
    ...
    async def _send_ntfy(self, alert: dict) -> None:
        ...
        try:
            async with session.post(url, data=alert["message"].encode(), headers=headers) as resp:
                if resp.status >= 400:
                    body = await resp.text()
                    logger.warning(f"ntfy returned {resp.status}: {body[:200]}")
        finally:
            if own_session:
                await session.close()
